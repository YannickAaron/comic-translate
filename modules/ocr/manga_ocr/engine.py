...
class MangaOCREngine(OCREngine):
    ...
    def __init__(self):
        ...
        self.project_root = os.path.abspath(
            os.path.join(self.current_file_dir, '..', '..', '..'))
    ...
    def initialize(self, device: str = 'cpu', expansion_percentage: int = 5) -> None:
        ...
        if self.model is None:
            ...
            manga_ocr_path = os.path.join(
                self.project_root, 'models/ocr/manga-ocr-base')
            self.model = MangaOcr(
                pretrained_model_name_or_path=manga_ocr_path, device=device)
