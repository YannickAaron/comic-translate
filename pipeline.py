...
class ComicTranslatePipeline:
    ...
    def load_box_coords(self, blk_list: List[TextBlock]):
        ...
        if self.main_page.image_viewer.hasPhoto() and blk_list:
            for blk in blk_list:
                ...
                rect_item = MoveableRectItem(
                    rect, self.main_page.image_viewer.photo)
    ...
    def manual_inpaint(self):
        ...
        if self.inpainter_cache is None or self.cached_inpainter_key != settings_page.get_tool_selection('inpainter'):
            device = 'mps' if settings_page.is_gpu_enabled() else 'cpu'
    ...
    def get_selected_block(self):
        ...
        blk = self.main_page.rect_item_ctrl.find_corresponding_text_block(
            srect_coords)
        return blk
    ...
    def _generate_image_hash(self, image):
        ...
        try:
            ...
            shape_str = str(image.shape) if hasattr(
                image, 'shape') else str(type(image))
            fallback_data = shape_str.encode() + str(image.dtype).encode() if hasattr(image,
                                                                                      'dtype') else b'fallback'
                    ...
                    if len(parts) >= 5:
                        ...
                        if (abs(target_x1 - cached_x1) <= tolerance and
                            ...
                            abs(target_y2 - cached_y2) <= tolerance and
                                ...
                                abs(target_angle - cached_angle) <= 1.0):
    ...
    def _get_cached_text_for_block(self, cache_key, block):
        ...
        if matched_id is not None:  # Block found in cache
            ...
            logger.debug(
                f"Available block IDs in cache: {list(cached_results.keys())}")
    ...
    def _get_translation_cache_key(self, image, source_lang, target_lang, translator_key, extra_context):
        ...
        context_hash = hashlib.md5(extra_context.encode(
        )).hexdigest() if extra_context else "no_context"
    ...
    def _cache_translation_results(self, cache_key, blk_list, processed_blk_list=None):
        ...
        try:
            ...
            logger.info(
                f"Cached translation results for {len(block_results)} blocks")
        except Exception as e:
    ...
    def _get_cached_translation_for_block(self, cache_key, block):
        ...
        matched_id, result = self._find_matching_translation_block_id(
            cache_key, block)
        ...
        if matched_id is not None:  # Block found in cache
            ...
            cached_results = self.translation_cache.get(cache_key, {})
            logger.debug(
                f"No cached translation found for block ID {block_id}")
            logger.debug(
                f"Available block IDs in cache: {list(cached_results.keys())}")
            return None  # Indicate block needs processing
    ...
    def _apply_cached_ocr_to_blocks(self, cache_key, block_list):
        ...
        for block in block_list:
            ...
            if cached_text is not None:  # Block found in cache
                # Apply cached text (could be empty string)
    ...
    def _apply_cached_translations_to_blocks(self, cache_key, block_list):
        ...
        for block in block_list:
            ...
            if cached_translation is not None:  # Block found in cache and source text matches
                # Apply cached translation (could be empty string)
        ...
        if self.main_page.image_viewer.hasPhoto() and self.main_page.image_viewer.rectangles:
            ...
            if single_block:
                ...
                if self._is_ocr_cached(cache_key):
                    ...
                    cached_text = self._get_cached_text_for_block(
                        cache_key, blk)
                ...
                if self._can_serve_all_blocks_from_ocr_cache(cache_key, self.main_page.blk_list):
                    ...
                    self._apply_cached_ocr_to_blocks(
                    ...
                    if self.main_page.blk_list:
                        ...
                        self._cache_ocr_results(
                            cache_key, self.main_page.blk_list)
                        logger.info("OCR completed and cached for %d blocks", len(
            ...
            if single_block:
                ...
                if self._is_translation_cached(translation_cache_key):
                    ...
                    translator.translate(
                        single_block_list, image, extra_context)
                    ...
                    if all_blocks_copy:
                        translator.translate(
                            all_blocks_copy, image, extra_context)
                        # Cache using the original blocks to maintain consistent IDs
                        self._cache_translation_results(
                            translation_cache_key, self.main_page.blk_list, all_blocks_copy)
                        cached_translation = self._get_cached_translation_for_block(
                            translation_cache_key, blk)
                        blk.translation = cached_translation
                        logger.info(
                ...
                if self._can_serve_all_blocks_from_translation_cache(translation_cache_key, self.main_page.blk_list):
                    ...
                    translator.translate(
                        self.main_page.blk_list, image, extra_context)
                    self._cache_translation_results(
                        translation_cache_key, self.main_page.blk_list)
                    logger.info("Translation completed and cached for %d blocks", len(
                        self.main_page.blk_list))
    ...
    def skip_save(self, directory, timestamp, base_name, extension, archive_bname, image):
        path = os.path.join(
            directory, f"comic_translate_{timestamp}", "translated_images", archive_bname)
        if not os.path.exists(path):
            os.makedirs(path, exist_ok=True)
        cv2.imwrite(os.path.join(
            path, f"{base_name}_translated{extension}"), image)
    ...
    def batch_process(self, selected_paths: List[str] = None):
        ...
        for index, image_path in enumerate(image_list):
            ...
            if state.get('skip', False):
                self.skip_save(directory, timestamp, base_name,
                               extension, archive_bname, image)
                self.log_skipped_image(
                    directory, timestamp, image_path, "User-skipped")
                continue

            # Text Block Detection
            self.main_page.progress_update.emit(
                index, total_images, 1, 10, False)
            ...
            if blk_list:
                ...
                try:
                    ...
                    self.main_page.image_skipped.emit(
                        image_path, "OCR", err_msg)
                    self.log_skipped_image(
                        directory, timestamp, image_path, reason)
                    continue
            else:
                self.skip_save(directory, timestamp, base_name,
                               extension, archive_bname, image)
                self.main_page.image_skipped.emit(
                    image_path, "Text Blocks", "")
                self.log_skipped_image(
                    directory, timestamp, image_path, "No text blocks detected")
                continue

            self.main_page.progress_update.emit(
                index, total_images, 3, 10, False)
            if self.main_page.current_worker and self.main_page.current_worker.is_cancelled:
                self.main_page.current_worker = None
                break
            ...
            inpaint_input_img = cv2.cvtColor(
                inpaint_input_img, cv2.COLOR_BGR2RGB)

            if export_settings['export_inpainted_image']:
                path = os.path.join(
                    directory, f"comic_translate_{timestamp}", "cleaned_images", archive_bname)
                if not os.path.exists(path):
                    os.makedirs(path, exist_ok=True)
                cv2.imwrite(os.path.join(
                    path, f"{base_name}_cleaned{extension}"), inpaint_input_img)

            self.main_page.progress_update.emit(
                index, total_images, 5, 10, False)
            if self.main_page.current_worker and self.main_page.current_worker.is_cancelled:
                self.main_page.current_worker = None
                break
            ...
            try:
                ...
                self._cache_translation_results(
                    translation_cache_key, blk_list)
            except Exception as e:
                # if it's an HTTPError, try to pull the "error_description" field
                if isinstance(e, requests.exceptions.HTTPError):
                    try:
            ...
            if export_settings['export_raw_text']:
                ...
                if not os.path.exists(path):
                    os.makedirs(path, exist_ok=True)
                file = open(os.path.join(path, os.path.splitext(
                    os.path.basename(image_path))[0] + "_raw.txt"), 'w', encoding='UTF-8')
                file.write(entire_raw_text)

            if export_settings['export_translated_text']:
                path = os.path.join(
                    directory, f"comic_translate_{timestamp}", "translated_texts", archive_bname)
                if not os.path.exists(path):
                    os.makedirs(path, exist_ok=True)
                file = open(os.path.join(path, os.path.splitext(os.path.basename(image_path))[
                            0] + "_translated.txt"), 'w', encoding='UTF-8')
                file.write(entire_translated_text)

            self.main_page.progress_update.emit(
                index, total_images, 7, 10, False)
            if self.main_page.current_worker and self.main_page.current_worker.is_cancelled:
                self.main_page.current_worker = None
                break
            ...
            sv_pth = os.path.join(
                render_save_dir, f"{base_name}_translated{extension}")
            ...
            self.main_page.progress_update.emit(
                index, total_images, 10, 10, False)
        ...
        if archive_info_list:
            ...
            for archive_index, archive in enumerate(archive_info_list):
                ...
                save_dir = os.path.join(
                    archive_directory, f"comic_translate_{timestamp}", "translated_images", archive_bname)
