...
class OCRFactory:
    ...
    def create_engine(cls, settings, source_lang_english: str, ocr_model: str) -> OCREngine:
        ...
        cache_key = cls._create_cache_key(
            ocr_model, source_lang_english, settings)
        ...
        engine = cls._create_new_engine(
            settings, source_lang_english, ocr_model)
    ...
    def _create_microsoft_ocr(settings) -> OCREngine:
        credentials = settings.get_credentials(
            settings.ui.tr("Microsoft Azure"))
        engine = MicrosoftOCR()
        engine.initialize(
    ...
    def _create_manga_ocr(settings) -> OCREngine:
        device = 'mps' if settings.is_gpu_enabled() else 'cpu'
